...
function Dryer (props) {
    ...
    const [timerDryerValue, setTimerDryer] = useState("");
    ...
    const [countdownDryerIntervalId, setCountdownDryerIntervalId] = useState(null); // Store the countdown interval ID
        ...
        try {
            ...
            if (!querySnapshot.empty) {
                ...
                if (machineDocSnap.exists()) {
                    ...
                    setTimerDryer(machineData.timer);
                    ...
                    if (userDocSnap.exists()) {
                        ...
                        if (walletID) {
                            ...
                            await updateDoc(walletDocRef, {
                                tokens: tokensValue - 6,
                                points: pointsValue + 6,
                            });
    ...
    const startCountdownDryer = async (startTimeDryer) => {
        const countdownDryerInterval = setInterval(async () => {
            const currentTime = new Date().getTime();
            const remainingTime = startTimeDryer - currentTime;

            if (remainingTime <= 0) {
                clearInterval(countdownDryerInterval);
                setTimerDryer(timerDryerValue);
                ...
                setTimerDryer(formattedTime);
            }
        }, 1000);
        
        setCountdownDryerIntervalId(countdownDryerInterval);
    };

    useEffect(() => {
        const storedStartTime = localStorage.getItem("countdownStartTimeDryer");
        const startTimeDryer = storedStartTime ? parseInt(storedStartTime) : null;

        if (startTimeDryer) {
            startCountdownDryer(startTimeDryer);
        }

        return () => {
            clearInterval(countdownDryerIntervalId);
        };
    }, []);
    ...
    const handleStartMachine = async () => {
        try {
            ...
            if (user) {
                ...
                if (!querySnapshot.empty) {
                    ...
                    const machineDocRef = querySnapshot.docs[0].ref;
                    const currentTime = new Date().getTime();
                    const etcTime = new Date(currentTime + 1800000).toLocaleTimeString("en-US", {
                        timeZone: "Asia/Kuala_Lumpur",
                        hour: "numeric",
                        minute: "numeric",
                        hour12: true,
        ...
        const startTimeDryer = new Date().getTime() + 120000; // Add 5 minutes (300,000 milliseconds) to the current time
        localStorage.setItem("countdownStartTimeDryer", startTimeDryer.toString());
        startCountdownDryer(startTimeDryer);
    };
            ...
            <div className="dryerSelected screen">
                <div className="dryerSelected-1">
                    ...
                    <div className="smallBoxDS">
                                ...
                                <div className="selectTemp">{selectTemperature}
                                    <div className="temperature">
                                        <input
                                            ...
                                            name="temperature"
                                            value="Low"
                                            checked={temperature === "Low"} // Check if the temperature state matches the value
                                    ...
                                    <div className="temperature">
                                        <input
                                            ...
                                            name="temperature"
                                            value="High"
                                            checked={temperature === "High"} // Check if the temperature state matches the value
                                            onChange={handleTemperatureChange}
                                        /> 
                                    High</div>
                                </div>
                        ...
                        <div className="timer">{timerDryerValue}</div>
                        {statusValue === "In Use" ? (
                            <>
                                {showStartButton && (
                                    <>
                                        <button className="buttonStart" onClick={handleStartMachine}>
                                            <div className="buttonInfoDS">{startNow}</div>
                                        </button>
                            ...
                            <button className="buttonPay" onClick={handleConfirmPayMachine}>
                                <div className="buttonInfoDS">{payNow}</div>
                            </button>
                        )}
                    </div>
